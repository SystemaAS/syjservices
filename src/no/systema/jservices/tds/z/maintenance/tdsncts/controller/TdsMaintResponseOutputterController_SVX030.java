...
@Controller
public class TdsMaintResponseOutputterController_SVX030 {
	private static Logger logger = LoggerFactory.getLogger(TdsMaintResponseOutputterController_SVX030.class.getName());
		...
		try{
			...
			if(userName!=null && !"".equals(userName)){
	            ...
	            if(dao.getTggnr()!=null && !"".equals(dao.getTggnr())){
					...
					list = this.svxghDaoServices.getList(dbErrorStackTrace);
				}
				//process result
				if (list!=null){
			...
			if(userName!=null && !"".equals(userName)){
				...
				if("D".equals(mode)){
					if(rulerLord.isValidInputForDelete(dao, userName, mode)){
						dmlRetval = this.svxghDaoServices.delete(dao, dbErrorStackTrace);
					}else{
						//write JSON error output
				  ...
				  if(rulerLord.isValidInput(dao, userName, mode)){
						...
						if("A".equals(mode)){
							...
							if(list!=null && list.size()>0){
								...
								dmlRetval = this.svxghDaoServices.insert(dao, dbErrorStackTrace);
							}
						}else if("U".equals(mode)){
							 dmlRetval = this.svxghDaoServices.update(dao, dbErrorStackTrace);
						}
